...
- 每次修改项目代码后，必须使用 `git add` 暂存变更，`git commit` 提交，并 `git push` 推送至 GitHub。
...
| `"h5"` | `data/h5_dataset.py:H5MMFiDataset` | Single HDF5 | `(3, 114, 10)` | global min-max |

Both return the same dict: `{"csi": (64,3,114), "kpts18": (18,2), "pcm": (19,36,36), "paf": (38,36,36), "meta": {...}}`

**HDF5 data flow** (server deployment):
1. Read raw CSI `(3, 114, 10)` from HDF5
2. Time resample `10 → 64` via `scipy.signal.resample`
3. Transpose to `(64, 3, 114)`
4. Global min-max normalize with pre-computed train-split bounds (stored as H5 attrs)
5. COCO17 `(17,2)` → OpenPose18 `(18,2)` keypoint conversion via `data/heatmap_gt.py:coco17_to_openpose18()`
6. Build PCM (19ch) and PAF (38ch) heatmap targets
...
- `csi` — `time_packets` (64), `rx_antennas` (3), `subcarriers` (114), `subcarrier_mode`, `normalize`
